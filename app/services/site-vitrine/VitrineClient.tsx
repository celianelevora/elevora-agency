...
export default function VitrineClient() {
  return (
    <>
      ...
      {/* 1+2 — HERO + POUR QUI : img-1 sombre, fond CONTINU (un seul tenant) */}
      <ServiceHero
        ...
        below={
          <>
            <div style={{ maxWidth: 820, marginBottom: 66 }}>
              <Reveal>
                <span className="svc-eyebrow" style={{ color: 'rgba(234,233,238,.72)' }}>Pour qui</span>
              </Reveal>
              <RevealText as="h2" text="Fait pour celles et ceux qui veulent être pris au sérieux." emphasis="sérieux." style={{ margin: '22px 0 0', color: '#EAE9EE', ...H2('clamp(33px,4.4vw,58px)') }} />
            </div>
      ...
      <SectionBg image={`${IMG}/img-2.webp`} scrim="light-strong" parallax={65} py={144} glow="blush" bgPosition="center 72%">
        <div style={{ maxWidth: 680, marginBottom: 58 }}>
          ...
          <RevealText as="h2" text="Un site, quatre exigences non négociables." emphasis="quatre" style={{ margin: '22px 0 22px', ...H2('clamp(32px,4.2vw,54px)') }} />
          <Reveal delay={0.15}>
            <p style={{ fontSize: 16.5, lineHeight: 1.75, color: 'var(--ink-soft)', maxWidth: 520 }}>{CONSTRUIT_INTRO}</p>
          </Reveal>
        </div>
        ...
        <div className="vit-crows">
          {CONSTRUIT.map((c, i) => (
            <Reveal key={c.t} delay={i * 0.08} className="vit-crow">
              ...
              <div>
                <h3 style={{ fontSize: 'clamp(22px,2.5vw,30px)', fontWeight: 500, color: 'var(--ink)', marginBottom: 9, lineHeight: 1.16 }}>{c.t}</h3>
                <p style={{ fontSize: 15, lineHeight: 1.7, color: 'var(--ink-soft)', maxWidth: 560 }}>{c.d}</p>
              </div>
      ...
      {/* 4 — NOTRE APPROCHE (img-3, sombre) — frise qui se dessine */}
      <SectionBg image={`${IMG}/img-3.webp`} scrim="dark-strong" parallax={50} py={144} glow="klein-pink">
        <div style={{ textAlign: 'center', maxWidth: 660, margin: '0 auto 74px' }}>
          ...
          <RevealText as="h2" text="De la première idée à la mise en ligne." emphasis="première" style={{ margin: '22px 0 0', color: '#EAE9EE', ...H2('clamp(32px,4.2vw,54px)') }} />
        </div>
          ...
          {APPROCHE.map((a, i) => (
            <Reveal key={a.k} delay={0.15 + i * 0.14} className="vit-tl__step">
              ...
              <div>
                ...
                <h3 style={{ fontSize: 'clamp(19px,1.9vw,23px)', fontWeight: 500, marginBottom: 9, lineHeight: 1.2, color: '#EAE9EE' }}>{a.t}</h3>
                <p style={{ fontSize: 14, lineHeight: 1.65, color: 'rgba(234,233,238,.72)' }}>{a.d}</p>
              </div>
            </Reveal>
          ))}
        </div>
          ...
          <div className="vit-stats">
            {SOCLE_STATS.map((stat, i) => (
              <Reveal key={stat.l} delay={i * 0.1} className="vit-stat">
                <div style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(52px,6.2vw,80px)', fontWeight: 500, letterSpacing: '-0.005em', color: i % 2 ? 'var(--pink)' : 'var(--klein)', lineHeight: 1, fontVariantNumeric: 'lining-nums tabular-nums' }}>
                  <Counter to={stat.v} suffix={stat.s} decimals={stat.d} />
                </div>
                <motion.div aria-hidden initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 1, delay: i * 0.1 + 0.35, ease: [0.22, 1, 0.36, 1] }} style={{ height: 2, width: 46, background: i % 2 ? 'var(--pink)' : 'var(--klein)', transformOrigin: 'left', margin: '15px 0 11px', borderRadius: 2 }} />
                <div style={{ fontSize: 13.5, color: 'var(--ink-muted)' }}>{stat.l}</div>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <SectionBg image={`${IMG}/img-5.webp`} scrim="dark-strong" parallax={45} py={144} glow="pink-klein">
        <div className="vit-price">
          <div>
            <Reveal>
              <span className="svc-eyebrow" style={{ color: 'rgba(234,233,238,.72)' }}>Tarif & délais</span>
            </Reveal>
            <RevealText as="h2" text="Un investissement clair, sans surprise." emphasis="clair," style={{ margin: '20px 0 38px', color: '#EAE9EE', ...H2('clamp(30px,3.8vw,50px)') }} />
            <Reveal delay={0.1}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 22 }}>
                ...
                <span style={{ fontSize: 14, color: 'rgba(234,233,238,.55)' }}>livré en</span>
          ...
          <div>
            <Reveal>
              <div style={{ fontSize: 12, letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 600, color: 'var(--klein-bright)', marginBottom: 4 }}>Tout est compris</div>
            </Reveal>
            <div>
              {INCLUS.map((it, i) => (
                <Reveal key={it} delay={i * 0.05} className="svc-inc svc-inc--dark">
                  <span className="svc-inc__check" aria-hidden>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--klein-bright)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </span>
                  <span style={{ fontSize: 15.5, lineHeight: 1.5, color: '#EAE9EE' }}>{it}</span>
                </Reveal>
              ))}
            </div>
          </div>
...
.vit-aud__glow { position: absolute; left: 0; right: 0; bottom: -30%; height: 70%; background: radial-gradient(ellipse at 50% 100%, rgba(43,108,196,.20), transparent 70%); opacity: 0; transition: opacity .5s; }
.vit-aud__col:hover .vit-aud__glow { opacity: 1; }
@media (max-width: 820px) {
  .vit-aud { grid-template-columns: 1fr; }
  .vit-aud__col { border-left: none; border-top: 0.5px solid rgba(234,233,238,.14); padding: 32px 0; }
  .vit-aud__col:first-child { border-top: none; padding-top: 0; }
}

/* Construit — lignes lumineuses pleine largeur */
.vit-crow { position: relative; display: grid; grid-template-columns: minmax(0,1fr) auto; gap: 18px 44px; align-items: center; padding: 30px 0 30px 0; border-top: 0.5px solid var(--line); transition: padding-left .45s cubic-bezier(.22,1,.36,1); }
...
.vit-crow__kw { font-family: var(--serif); font-style: italic; font-size: clamp(22px,2.6vw,33px); color: rgba(26,26,46,.20); white-space: nowrap; transition: color .4s; }
.vit-crow:hover .vit-crow__kw { color: var(--klein); }
...
.vit-tl__track { background: rgba(234,233,238,.18); }
...
@media (max-width: 860px) {
  ...
  .vit-tl__step { text-align: left; display: grid; grid-template-columns: 50px 1fr; gap: 22px; padding: 24px 0; border-top: 0.5px solid rgba(234,233,238,.14); align-items: start; }
...
/* Inclus sur fond sombre (partie tarif) */
.svc-inc.svc-inc--dark { border-top-color: rgba(234,233,238,.14); }
.svc-inc--dark .svc-inc__check { border-color: rgba(234,233,238,.28); }
`;
